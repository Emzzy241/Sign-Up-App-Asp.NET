...
namespace Sign_up
{
  class Program
  {
    static void Main(string[] args)
    {
      ...
      builder.Services.AddDbContext<Sign_upContext>(
      ...
      /* New code below!!
        ...
        We chain two more method calls to the Identity service that we set up: .AddEntityFrameworkStores<Sign_upContext>() and .AddDefaultTokenProviders();. The first method ensures that the Identity user data is saved via EF Core to our database (as represented by the Sign_upContext class). The second method sets up Identity's providers for tokens, which are created during password reset or two factor authentication, for example. Note that we won't go over how to implement either of those two things in the coursework, and you are encouraged to look into them on your own.
      ...
      */
      ...
      builder.Services.AddIdentity<ApplicationUser, IdentityRole>()
                .AddEntityFrameworkStores<Sign_upContext>()
