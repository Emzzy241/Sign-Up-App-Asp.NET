...
using Sign_up.ViewModels;

namespace Sign_up.Controllers
{
  public class AccountController : Controller
  {
    private readonly Sign_upContext _db;
    private readonly UserManager<ApplicationUser> _userManager;
    private readonly SignInManager<ApplicationUser> _signInManager;

    public AccountController (UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, Sign_upContext db)
    {
    ...
    public async Task<IActionResult> Login( LoginViewModel model)
    {
      ...
      else
      {
          ...
          {
              ModelState.AddModelError("", "There is something wrong with your email or password. Please check them and try again");
              return View(model);
          }
      }
    }
    ...
    /*
        ...
        We also include a using directive for Sign_up.ViewModels. The ViewModels namespace will include view models that we haven't created yet, like the RegisterViewModel. Note that we haven't covered this yet, and we will in the very next lesson.
        ...
        emember that _userManager represents Identity's UserManager<TUser> class, which was injected as a service into the AccountController. The UserManager<TUser> class has a method called CreateAsync(). As explained in the documentation, this method will create a user with the provided password.
